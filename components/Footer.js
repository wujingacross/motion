...
const Footer = ({ footerData }) => {
  ...
  return <footer className="bg-darkblue lg:bg-transparent lg:bg-footer lg:bg-no-repeat lg:bg-left-bottom lg:min-h-[738px] pt-12 lg:pt-0 relative">
    <motion.div
      ...
      <motion.div variants={fadeInUp} className="flex flex-col lg:flex-row items-center lg:pr-[95px] lg:gap-x-[95px] gap-y-10 lg:gap-y-0">
        ...
        <form className="w-full max-w-[550px] bg-white flex-1 rounded-[10px] lg:order-2 flex flex-col px-[35px] lg:px-[75px] py-[25px] lg:py-[52px] space-y-[40px] drop-shadow-primary">
          ...
          <div className="flex flex-col">
            <label className="font-light mb-[10px]" htmlFor='email'>{form.labelEmail}</label>
            <input className="input" type="text" id="email" placeholder={form.placeholderEmail} />
          </div>
      ...
      <motion.div variants={fadeInUp} className="py-[120px] flex flex-col lg:flex-row justify-between lg:pr-24">
        <div className="mb-6 mx-auto lg:mb-9 lg:mx-0">
          <Link href="/">
            <a>
              <Image src={logo} alt="logo" width={170} height={41}></Image>
            </a>
          </Link>
        </div>
        <ul className="flex flex-col lg:flex-row items-center gap-y-4 lg:gap-y-0 gap-x-[100px] text-center">
          {links.map((item, idx) => {
            return <li key={idx} className="text-white font-light hover:text-accent transition">
              <Link href={item.href}>
                {item.name}
              </Link>
            </li>
